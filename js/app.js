...
App = {
    ...
    init: function() {
	    ...
	    $("#menu").effect("drop", {mode: "hide", direction: "up"});
            //$(".page, .ui-page-active").show();
	    $("body").scrollTop(0);
            menuStatus = false;
            var p = $(this).parent();
	...
	$(window).resize();
	App.navInit();
	if ($(".ui-page-active .ui-content").attr("disable-back-button") == "true")
	    $(".back-trigger").fadeOut();
	else
	    $(".back-trigger").fadeIn();
	...
	$(".menu-trigger:not(.vclicked)").addClass("vclicked").bind("vclick", function() {
	    if (!$(".ui-page-active .main-menu").is(":visible")) {
		$("span.square-button", this).html($(this).attr("othertext"));
		$(".ui-page-active .main-menu").slideDown();
		$(".white-overlay").fadeIn();
		//$(".ui-content").slideUp();
	    } else {
		$("span.square-button", this).html($(this).attr("defaulttext"));
		$(".ui-page-active .main-menu").slideUp();
		$(".white-overlay").fadeOut();
		//$(".ui-content").slideDown();
	    }
	});
	
	$(".menu-button:not(.vclicked)").addClass("vclicked").bind("click", function() {
	    $(".menu-trigger").click();
	    clearInterval(App.setPageTitleInterval);
	    var me = this;
	    App.setPageTitleInterval = setInterval(function() {
		$(".page-title").text($(".button-label", me).text());
	    }, 1000);
	...
	/*
        $("a.showMenu").click(function(){
            ...
            if(menuStatus != true){
                $("#menu").effect("drop", {mode: "show", direction: "down"}, 400, function() {
		    //$(".page, .ui-page-active").hide();
		});
